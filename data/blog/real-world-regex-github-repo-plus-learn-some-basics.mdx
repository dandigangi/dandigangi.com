...
tags: ['code', 'repos']
draft: false
summary: 'Collection of regex (regular expressions) including explanations that come up during my work to make life easier so you can learn them too.'
...
### When You May Need Regex

Finding and manipulating strings will come up often in your career. It is slightly different in some ways as a manager but the overlap is still high. First we'll look at a basic email example followed by the real world example I encountered.
...
The last step after returning successful searches was manipulating the strings to be valid emails. Here's an updated version showing a capture `()`. You can see `$1` is the captured match of the first part of each email. `,look i changed this string` is added on and now we have our final output. Think of it as a code based find & replace.
...
All in all we ended w/ a clean list that was handed over to product and they we're good to go!
